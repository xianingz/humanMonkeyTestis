...
hum.spg.extra.genes[which(hum.corr.mac.max < 0.8),]$macclu <- 0
```
...
mac.spg.extra.genes[which(mac.corr.hum.max < 0.8),]$humclu <- 0
```
...
hum.spg.extra.mou.genes[which(hum.corr.mou.max < 0.8),]$mouclu <- 0
```
...
mou.spg.extra.hum.genes[which(mou.corr.hum.max < 0.8),]$humclu <- 0
```
...
mac.spg.extra.mou.genes[which(mac.corr.mou.max < 0.8),]$mouclu <- 0
```
...
mou.spg.extra.mac.genes[which(mou.corr.mac.max < 0.8),]$macclu <- 0
```
...
```{r}
write.table(hum.mac.spg.genes.orth[,c(1,2,4,5)], quote = FALSE, sep = "\t", file = "~/storage/HumanMacaqueMouseCompAnaysis/interdata/SPG_GeneKmeans/hum_mac.spg.genelist.txt", row.names = FALSE)
write.table(hum.mou.spg.genes.orth[,c(1,3,4,6)], quote = FALSE, sep = "\t", file = "~/storage/HumanMacaqueMouseCompAnaysis/interdata/SPG_GeneKmeans/hum_mou.spg.genelist.txt", row.names = FALSE)
write.table(mac.mou.spg.genes.orth[,c(2,3,5,6)], quote = FALSE, sep = "\t", file = "~/storage/HumanMacaqueMouseCompAnaysis/interdata/SPG_GeneKmeans/mac_mou.spg.genelist.txt", row.names = FALSE)
```
