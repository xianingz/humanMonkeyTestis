...
```{r fig.width=5,fig.height=5}
plot(log(mac.mean), mac.max.cor) + abline(v=-3) + abline(h=0.8)
```


```{r}
kmeans.mac.genes.add <- mac.hvg.warped.kmeans.genes
for(i in hum.extra.genes$mmulatta_homolog_associated_gene_name){
  if(mac.max.cor[i] >= 0.8 & log(mac.mean[i])>-3){
    max.cor.gene <- names(which.max(max.extra.cor[,i]))
    max.cor.stage <- kmeans.mac.genes.add$stages[kmeans.mac.genes.add$genes==max.cor.gene]
