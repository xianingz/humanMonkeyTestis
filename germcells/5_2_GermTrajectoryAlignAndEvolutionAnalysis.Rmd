...
```{r}
dge.hum.cen <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/hum.201.centroid.txt", sep = "\t", row.names = 1)
dge.mac.cen <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/mac.201.centroid.txt", sep = "\t", row.names = 1)
dge.mou.cen <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/mou.201.centroid.txt", sep = "\t", row.names = 1)
ca.cen.int.600 <- cbind(dge.hum.cen[int.genes$external_gene_name.x,], dge.mac.cen[int.genes$mmulatta_homolog_associated_gene_name,],  dge.mou.cen[int.genes$mmusculus_homolog_associated_gene_name,])
...
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/germ.align.pca.pc1_2.pdf", width = 7, height = 5)
ggplot(data.frame(ca.cen.int.600.pca$x, species=factor(colnames(ca.cen.int.600)), num=c(dim(dge.hum.spg.raw)[2],hum.lens, dim(dge.mac.spg.raw)[2],mac.lens, dim(dge.mou.spg.raw)[2], mou.lens),traj=c(2,rep(1,200),rep(c(2,rep(1,200)),2))), aes(PC1,PC2)) + geom_point(aes(colour=species, shape=factor(traj), size=sqrt(num)))+ scale_color_manual(values = c("#e35a97","#5a97e3","#5ae3a6"))
#dev.off()
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/germ.align.pca.pc1_3.pdf", width = 7, height = 5)
ggplot(data.frame(ca.cen.int.600.pca$x, species=factor(colnames(ca.cen.int.600)), num=c(dim(dge.hum.spg.raw)[2],hum.lens, dim(dge.mac.spg.raw)[2],mac.lens, dim(dge.mou.spg.raw)[2], mou.lens),traj=c(2,rep(1,200),rep(c(2,rep(1,200)),2))), aes(PC1,PC3)) + geom_point(aes(colour=species, shape=factor(traj), size=sqrt(num)))+ scale_color_manual(values = c("#e35a97","#5a97e3","#5ae3a6"))
#dev.off()
```
...
hum.orthos <- humMacMou.AllGene[humMacMou.AllGene$mmulatta_homolog_ensembl_gene!="" | humMacMou.AllGene$mmusculus_homolog_ensembl_gene!="",]
hum.spe <- hum.spe[!hum.spe$external_gene_name.x %in% hum.orthos$external_gene_name.x,]
...
mou.orthos <- mouHumMac.AllGene[mouHumMac.AllGene$mmulatta_homolog_ensembl_gene!=""|mouHumMac.AllGene$hsapiens_homolog_ensembl_gene!="",]
mou.spe <- mou.spe[!mou.spe$external_gene_name.x %in% mou.orthos$external_gene_name.x,]
...
mac.orthos <- macHumMou.AllGene[macHumMou.AllGene$mmusculus_homolog_ensembl_gene!=""|macHumMou.AllGene$hsapiens_homolog_ensembl_gene!="",]
mac.spe <- mac.spe[!mac.spe$external_gene_name.x %in% mac.orthos$external_gene_name.x,]
...
hum.mou.orth <- humMacMou.AllGene[humMacMou.AllGene$mmusculus_homolog_ensembl_gene!="",]
hummac.spe <- hummac.spe[!hummac.spe$external_gene_name.x %in% hum.mou.orth$external_gene_name.x,]
```

### filter by genes existing in our dataset.
```{r}
##for monkey replace empty gene name with corresponding ensembl gene id.
hummac.spe$mmulatta_homolog_associated_gene_name[hummac.spe$mmulatta_homolog_associated_gene_name==""]=hummac.spe$mmulatta_homolog_ensembl_gene[hummac.spe$mmulatta_homolog_associated_gene_name==""]

hum.hummac.spe.coding <- hummac.spe[hum_type[hummac.spe$ensembl_gene_id, ]$gene_biotype=="protein_coding",]
hum.hummac.spe.coding <- intersect(hum.hummac.spe.coding$external_gene_name.x,rownames(dge.hum.germ.norm))

mac.hummac.spe.coding <- hummac.spe[mac_type[hummac.spe$mmulatta_homolog_ensembl_gene, ]$gene_biotype=="protein_coding",]
mac.hummac.spe.coding <-intersect(mac.hummac.spe.coding$mmulatta_homolog_associated_gene_name, rownames(dge.mac.germ.norm))
```

###. primate-specific genes
####. percentage of total expression
```{r fig.width=7,fig.height=5}
hum.hummac.coding.perc <- apply(hum.201.cen[hum.hummac.spe.coding,],2,sum)/apply(hum.201.cen[hum.all.det.coding,],2,sum)
mac.hummac.coding.perc <- apply(mac.201.cen[mac.hummac.spe.coding,],2,sum)/apply(mac.201.cen[mac.all.det.coding,],2,sum)
...
mou.hum <- mouHumMac.AllGene[mouHumMac.AllGene$hsapiens_homolog_ensembl_gene!="",]
moumac.spe <- moumac.spe[!moumac.spe$external_gene_name.x %in% mou.hum$external_gene_name.x,]
```

### moumac specific genes
```{r}
##replace empty monkey gene name with corresponding ensembl gene id 
moumac.spe$mmulatta_homolog_associated_gene_name[moumac.spe$mmulatta_homolog_associated_gene_name==""] = moumac.spe$mmulatta_homolog_ensembl_gene[moumac.spe$mmulatta_homolog_associated_gene_name==""]
mou.moumac.spe.coding <- moumac.spe[mou_type[moumac.spe$ensembl_gene_id, ]$gene_biotype=="protein_coding",]
mou.moumac.spe.coding <- intersect(mou.moumac.spe.coding$external_gene_name.x,rownames(dge.mou.germ.norm))

mac.moumac.spe.coding <- moumac.spe[mac_type[moumac.spe$mmulatta_homolog_ensembl_gene, ]$gene_biotype=="protein_coding",]
mac.moumac.spe.coding <-intersect(mac.moumac.spe.coding$mmulatta_homolog_associated_gene_name, rownames(dge.mac.germ.norm))
```

###Mouse-monkey specific genes
#### percentage of total expression
```{r fig.width=7,fig.height=5}
mou.moumac.coding.perc <- apply(mou.201.cen[mou.moumac.spe.coding,],2,sum)/apply(mou.201.cen[mou.all.det.coding,],2,sum)
mac.moumac.coding.perc <- apply(mac.201.cen[mac.moumac.spe.coding,],2,sum)/apply(mac.201.cen[mac.all.det.coding,],2,sum)
...
hum.mac.orth <- humMacMou.AllGene[humMacMou.AllGene$mmulatta_homolog_ensembl_gene!="",]
hummou.spe <- hummou.spe[!hummou.spe$external_gene_name.x %in% hum.mac.orth$external_gene_name.x,]
```

####filtered by genes that have expression in our datasets.
```{r}
mou.hummou.spe.coding <- hummou.spe[mou_type[hummou.spe$mmusculus_homolog_ensembl_gene, ]$gene_biotype=="protein_coding",]
mou.hummou.spe.coding <- intersect(mou.hummou.spe.coding$mmusculus_homolog_associated_gene_name,rownames(dge.mou.germ.norm))
hum.hummou.spe.coding <- hummou.spe[hum_type[hummou.spe$ensembl_gene_id, ]$gene_biotype=="protein_coding",]
hum.hummou.spe.coding <-intersect(hum.hummou.spe.coding$external_gene_name.x, rownames(dge.hum.germ.norm))
```

####human-mouse specific genes
#### percentage of total expression.
```{r fig.width=7,fig.height=5}
mou.hummou.coding.perc <- apply(mou.201.cen[mou.hummou.spe.coding,],2,sum)/apply(mou.201.cen[mou.all.det.coding,],2,sum)
hum.hummou.coding.perc <- apply(hum.201.cen[hum.hummou.spe.coding,],2,sum)/apply(hum.201.cen[hum.all.det.coding,],2,sum)
...
##replace empty monkey symbol with ensembl id
humMacMouOrth.1to1$mmulatta_homolog_associated_gene_name[humMacMouOrth.1to1$mmulatta_homolog_associated_gene_name==""] = humMacMouOrth.1to1$mmulatta_homolog_ensembl_gene[humMacMouOrth.1to1$mmulatta_homolog_associated_gene_name==""]
```
...
mac_type$external_gene_name[mac_type$external_gene_name==""]=mac_type$ensembl_gene_id[mac_type$external_gene_name==""]
...
#remove 1-1-1 orthologs
mac.other.othos <- setdiff(mac.other.othos, humMacMouOrth.1to1.mac.det.coding)
##remove 8 genes that are in both 1-1-1 and moumac specific from moumac specific and keep them in 1-1-1
mac.moumac.spe.coding <- setdiff(mac.moumac.spe.coding, humMacMouOrth.1to1.mac.det.coding)
##remove 2 genes that are both in hummac and moumac from moumac
mac.moumac.spe.coding <- setdiff(mac.moumac.spe.coding,mac.hummac.spe.coding)
```
8 genes like "PISD" are both in 1-1-1 and moumac specific genes.
2 genes line "SYS1" are both in hummac and moumac genes list.
...
#remove 1-1-1 orthologs
mou.other.othos <- setdiff(mou.other.othos, humMacMouOrth.1to1.mou.det.coding)
##remove two genes that are both in 1-1-1 and hummou specific from hummou specific and keep them in 1-1-1
mou.hummou.spe.coding <- setdiff(mou.hummou.spe.coding, humMacMouOrth.1to1.mou.det.coding)
```
Two genes both in 1-1-1 and hummou specific "St6galnac2" "Jakmip1"
```{r}
...
#### percentage of total expression
```{r fig.width=7,fig.height=5}
hum.other.othos.perc <- apply(hum.201.cen[hum.other.othos,],2,sum)/apply(hum.201.cen[hum.all.det.coding,],2,sum)
mac.other.othos.perc <- apply(mac.201.cen[mac.other.othos,],2,sum)/apply(mac.201.cen[mac.all.det.coding,],2,sum)
mou.other.othos.perc <- apply(mou.201.cen[mou.other.othos,],2,sum)/apply(mou.201.cen[mou.all.det.coding,],2,sum)
...
```{r fig.width=7,fig.height=5}
hum.spe.coding.perc <- apply(hum.201.cen[hum.spe.coding,],2,sum)/apply(hum.201.cen[hum.all.det.coding,],2,sum)
mac.spe.coding.perc <- apply(mac.201.cen[mac.spe.coding,],2,sum)/apply(mac.201.cen[mac.all.det.coding,],2,sum)
mou.spe.coding.perc <- apply(mou.201.cen[mou.spe.coding,],2,sum)/apply(mou.201.cen[mou.all.det.coding,],2,sum)

pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/spe.specific.exp.pdf",width = 7, height = 5)
ggplot(data.frame(stages=c(1:201), orth=c(hum.spe.coding.perc, mac.spe.coding.perc, mou.spe.coding.perc), num=c(dim(dge.hum.spg.raw)[2],hum.lens, dim(dge.mac.spg.raw)[2],mac.lens, dim(dge.mou.spg.raw)[2], mou.lens),traj=c(2,rep(1,200),rep(c(2,rep(1,200)),2)), spe=c(rep("Human",201),rep("Monkey",201),rep("Mouse",201))), aes(stages,orth)) + geom_point(aes(colour=spe, shape=factor(traj), size=sqrt(num))) + scale_color_manual(values = c("#e35a97","#5a97e3","#5ae3a6")) + xlab("Pseudo points") + ylab("Percentage of total expression") + ggtitle("Species specific")
dev.off()
```
...
```{r fig.width=7,fig.height=5}
hum.111orth.coding.perc <- apply(hum.201.cen[humMacMouOrth.1to1.hum.det.coding,],2,sum)/apply(hum.201.cen[hum.all.det.coding,],2,sum)
mac.111orth.coding.perc <- apply(mac.201.cen[humMacMouOrth.1to1.mac.det.coding,],2,sum)/apply(mac.201.cen[mac.all.det.coding,],2,sum)
mou.111orth.coding.perc <- apply(mou.201.cen[humMacMouOrth.1to1.mou.det.coding,],2,sum)/apply(mou.201.cen[mou.all.det.coding,],2,sum)

pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/111orth.separate.coding.exp.pdf",width = 7, height = 5)
...
```{r fig.width=7,fig.height=5}
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/germ.align.principlecurve.pdf", width = 7, height = 5)
ggplot(data.frame(ca.cen.int.600.pca$x, species=factor(colnames(ca.cen.int.600)), num=c(dim(dge.hum.spg.norm)[2],hum.lens, dim(dge.mac.spg.norm)[2],mac.lens, dim(dge.mou.spg.norm)[2], mou.lens),traj=c(2,rep(1,200),2,rep(1,142), rep(3,58),2, rep(1,200))), aes(PC1,PC3)) + geom_point(aes(colour=species, shape=factor(traj), size=sqrt(num))) + scale_shape_manual(values = c(20, 17, 3)) + geom_line(data=data.frame(fit$s), aes(PC1, PC3)) + geom_segment(aes(x=x,y=y,xend=xend,yend=yend), colour="grey", size=0.2, data=data.frame(x=ca.cen.int.600.pca$x[rmmac,"PC1"], y=ca.cen.int.600.pca$x[rmmac,"PC3"], xend=fit$s[,"PC1"], yend=fit$s[,"PC3"])) + scale_color_manual(values=c("#e3a65a","#5a97e3","#e35a97")) + labs(color='Species') + labs(size='#Cells')  + guides(colour = guide_legend(override.aes = list(size=4)), shape=FALSE)
#dev.off()
```
...
for(i in c(1:20)){
  ...
  hum.cells <- c(hum.cells, cells)
  hum.labels <- c(hum.labels, rep(i, length(cells)))
  hum.20.warped.cen[,i] <- apply(dge.hum.germ.norm[,cells],1,mean)
}
hum.cells.lables <- data.frame(cells=hum.cells, labels=hum.labels)
...
for(i in c(1:20)){
  ...
  mac.cells <- c(mac.cells, cells)
  mac.labels <- c(mac.labels, rep(i, length(cells)))
  mac.20.warped.cen[,i] <- apply(dge.mac.germ.norm[,cells],1,mean)
}
mac.cells.lables <- data.frame(cells=mac.cells, labels=mac.labels)
...
for(i in c(1:20)){
  ...
  mou.cells <- c(mou.cells, cells)
  mou.labels <- c(mou.labels, rep(i, length(cells)))
  mou.20.warped.cen[,i] <- apply(dge.mou.germ.norm[,cells],1,mean)
}
mou.cells.lables <- data.frame(cells=mou.cells, labels=mou.labels)
```
...
#### Write the 20 stages labels
```{r}
write.table(hum.cells.lables,file="~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_PrincipleCurve_20Centroids/Hum.20.cells.labels.txt",quote = FALSE, row.names = FALSE, col.names = TRUE,sep = "\t")
write.table(mac.cells.lables,file="~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_PrincipleCurve_20Centroids/Mac.20.cells.labels.txt",quote = FALSE, row.names = FALSE, col.names = TRUE,sep = "\t")
write.table(mou.cells.lables,file="~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_PrincipleCurve_20Centroids/Mou.20.cells.labels.txt",quote = FALSE, row.names = FALSE, col.names = TRUE,sep = "\t")
```
...
```{r fig.width=8, fig.height=4}
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/flow_width.pdf", width = 8, height = 4)
dat <- data.frame(traj = c(hum.26.lens,mac.26.lens,mou.26.lens), spe=c(rep("Human",26),rep("Monkey",26),rep("Mouse",26)), pt=rep(c(1:26),3))
ggplot(dat, aes(pt, factor(spe))) + scale_size_continuous(range=c(1,10)) + geom_point(aes(col=factor(spe),size=sqrt(traj))) + labs(x="Pseudo-time from principle curve",y="Species") + geom_vline(xintercept = 6.5)
#dev.off()
```

###Dot plot adding Monkey extra
```{r fig.width=8, fig.height=4}
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/flow_width.pdf", width = 8, height = 4)
mac.extra <- length(unlist(lapply(c(143:200), function(x) hummac.mapping$queryAssign[[x]])))
dat <- data.frame(traj = c(hum.26.lens,NA,mac.26.lens,mac.extra,mou.26.lens,NA), spe=c(rep("Human",27),rep("Monkey",27),rep("Mouse",27)), pt=rep(c(1:27),3))
ggplot(dat, aes(pt, factor(spe))) + scale_size_continuous(range=c(1,10)) + geom_point(aes(col=factor(spe),size=sqrt(traj))) + labs(x="Pseudo-time from principle curve",y="Species") + geom_vline(xintercept = 6.5, linetype="dotted") + geom_vline(xintercept = 26.5, linetype="dotted") + labs(color='Species') + labs(size='#Cells') + scale_color_manual(values=c("#e3a65a","#5a97e3","#e35a97")) + guides(colour = guide_legend(override.aes = list(size=4)))
#dev.off()
```
...
```{r fig.width=5,fig.height=5}
plot(mac.gen$mean, mac.gen$varmean, pch=16, cex=0.5)
points(mac.gen[hum.extra.genes$mmulatta_homolog_associated_gene_name,]$mean,mac.gen[hum.extra.genes$mmulatta_homolog_associated_gene_name,]$varmean, col="red", pch=16, cex=0.5)
abline(v = 0.2)
abline(h = 0.2)
```
...
"#abb4ff",
"#b28e00")
ha = rowAnnotation(stages = mac.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(mac.cen.std[as.character(mac.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, left_annotation = ha)
```
...
"#abb4ff",
"#b28e00")
ha = rowAnnotation(stages = hum.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(hum.cen.std[as.character(hum.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, right_annotation = ha)
```

##20190816 Add extra genes from human that are not found in Monkey HVGs to the plot
#Connector lines
```{r}
kmeans.hum.genes <- humMacMouOrth.1to1.in[humMacMouOrth.1to1.in$external_gene_name.x %in% hum.hvgs,]

hum.extra.genes <- kmeans.hum.genes[!kmeans.hum.genes$mmulatta_homolog_associated_gene_name %in% mac.hvgs,]

max.extra.cor <- cor(t(mac.cen.std[mac.hvgs,]),t(mac.cen.std[hum.extra.genes$mmulatta_homolog_associated_gene_name,]), method = "spearman")
mac.max.cor <- apply(max.extra.cor,2,max)
mac.mean <- apply(mac.cen[hum.extra.genes$mmulatta_homolog_associated_gene_name,],1,mean)
```

```{r fig.width=5,fig.height=5}
plot(log(mac.mean), mac.max.cor) + abline(h=0.8)
```


```{r}
kmeans.mac.genes.add <- data.frame()
for(i in hum.extra.genes$mmulatta_homolog_associated_gene_name){
  if(mac.max.cor[i] >= 0.8){
    max.cor.gene <- names(which.max(max.extra.cor[,i]))
    max.cor.stage <- mac.hvg.warped.kmeans.genes$stages[mac.hvg.warped.kmeans.genes$genes==max.cor.gene]
    kmeans.mac.genes.add <- rbind(kmeans.mac.genes.add, data.frame(genes=i, stages=max.cor.stage))
  }else{
    kmeans.mac.genes.add <- rbind(kmeans.mac.genes.add, data.frame(genes=i, stages=7))
  }
}
kmeans.mac.genes.add <- kmeans.mac.genes.add[order(kmeans.mac.genes.add$stages),]
```

```{r}
kmeans.mac.genes <- humMacMouOrth.1to1.in[humMacMouOrth.1to1.in$mmulatta_homolog_associated_gene_name %in% mac.hvgs,]

mac.extra.genes <- kmeans.mac.genes[!kmeans.mac.genes$external_gene_name.x %in% hum.hvgs,]

hum.extra.cor <- cor(t(hum.cen.std[hum.hvgs,]),t(hum.cen.std[mac.extra.genes$external_gene_name.x,]), method = "spearman")
hum.max.cor <- apply(hum.extra.cor,2,max)
hum.mean <- apply(hum.cen[mac.extra.genes$external_gene_name.x,],1,mean)
```

```{r fig.width=5,fig.height=5}
plot(log(hum.mean), hum.max.cor) + abline(h=0.8)
```

```{r}
kmeans.hum.genes.add <- data.frame()
for(i in mac.extra.genes$external_gene_name.x){
  if(hum.max.cor[i] >= 0.8){
    max.cor.gene <- names(which.max(hum.extra.cor[,i]))
    max.cor.stage <- hum.hvg.warped.kmeans.genes$stages[hum.hvg.warped.kmeans.genes$genes==max.cor.gene]
    kmeans.hum.genes.add <- rbind(kmeans.hum.genes.add, data.frame(genes=i, stages=max.cor.stage))
  }else{
    kmeans.hum.genes.add <- rbind(kmeans.hum.genes.add, data.frame(genes=i, stages=7))
  }
}
kmeans.hum.genes.add <- kmeans.hum.genes.add[order(kmeans.hum.genes.add$stages),]
```
...
```{r}
pos.hum.mac <- c()
for(i in c(1:dim(kmeans.hum.genes)[1])){
  hum.gen <- kmeans.hum.genes$external_gene_name.x[i]
  mac.gen <- kmeans.hum.genes$mmulatta_homolog_associated_gene_name[i]
  x = which(hum.hvg.warped.kmeans.genes$genes == hum.gen)
  y = which(kmeans.mac.genes.add$genes == mac.gen)
  pos.hum.mac <- rbind(pos.hum.mac,c(x,y,hum.gen, mac.gen))
}
pos.hum.mac <- data.matrix(pos.hum.mac)
colnames(pos.hum.mac) = c("hum.pos","mac.pos", "hum.gen","mac.gen")
...
pos.hum.mac <- c()
for(i in c(1:dim(hum.kmeans.genes.ord)[1])){
  hum.gen <- hum.kmeans.genes.ord$genes[i]
  mac.gen <- kmeans.hum.genes$mmulatta_homolog_associated_gene_name[kmeans.hum.genes$external_gene_name.x==hum.gen]
  x = which(hum.kmeans.genes.ord$genes == hum.gen)
  y = which(kmeans.mac.genes.add$genes == mac.gen)
  pos.hum.mac <- rbind(pos.hum.mac,c(x,y,hum.gen, mac.gen))
}
pos.hum.mac <- data.matrix(pos.hum.mac)
...
rownames(pos.hum.mac) <- pos.hum.mac[,"mac.gen"]
mac.kmeans.genes.ord <- kmeans.mac.genes.add
for(i in c(1:7)){
  genes <- kmeans.mac.genes.add$genes[kmeans.mac.genes.add$stages==i]
  ogenes <- as.character(genes[!genes %in% pos.hum.mac[,"mac.gen"]])
  genes <- pos.hum.mac[,"mac.gen"][pos.hum.mac[,"mac.gen"] %in% genes]
  genes <- genes[order(as.numeric(pos.hum.mac[genes,"hum.pos"]))]
  mac.kmeans.genes.ord$genes[kmeans.mac.genes.add$stages==i] = c(genes,ogenes)
}
```

```{r fig.width=4, fig.height=10}
#pdf("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/Coonector.humman.hum.pdf", width = 4, height = 10)
#tiff("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/Coonector.humman.hum.tiff", units="in", width=4, height=10, res=1200)
cols <- c("#fabc3c",
"#a64dd4",
"#14e072",
"#d9014a",
...
"#b28e00",
"grey")
plot(1,xlim=c(1,2),ylim=c(0,1200),type="n",axes=FALSE,ann=FALSE)

for(i in c(1:dim(kmeans.hum.genes)[1])){
#for(i in c(1:10)){ 
  x = which(hum.kmeans.genes.ord$genes == kmeans.hum.genes$external_gene_name.x[i])
  y = which(mac.kmeans.genes.ord$genes == kmeans.hum.genes$mmulatta_homolog_associated_gene_name[i])
  #print(kmeans.hum.mac.genes$external_gene_name.x[i])
  #print(kmeans.hum.mac.genes$mmusculus_homolog_associated_gene_name[i])
  x = 1200-x*1000/dim(hum.hvg.warped.kmeans.genes)[1]
  y = 1200-y*1200/dim(kmeans.mac.genes.add)[1]
  #print(x)
  #print(y)
  segments(1,x,2,y,lwd = 0.1,col = "grey60")
}
points(data.frame(x=rep(1,dim(hum.hvg.warped.kmeans.genes)[1]),y=1200-c(1:dim(hum.hvg.warped.kmeans.genes)[1])*1000/dim(hum.hvg.warped.kmeans.genes)[1]),col=cols[hum.kmeans.genes.ord$stages], pch=15, cex=0.8)
points(data.frame(x=rep(2,dim(kmeans.mac.genes.add)[1]),y=1200-c(1:dim(kmeans.mac.genes.add)[1])*1200/dim(kmeans.mac.genes.add)[1]),col=cols[mac.kmeans.genes.ord$stages], pch=15, cex=0.8)
#segments(1,0,10,0,col = "red")
#dev.off()
```
...
"#b28e00",
"grey")
ha = rowAnnotation(stages = mac.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6], "7" = cols[7])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(mac.cen.std[as.character(mac.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, left_annotation = ha)
```
...
```{r fig.width=4, fig.height=10}
#tiff("~/storage/HumanMacaqueMouseCompAnaysis/interdata/plots/Coonector.hummau.hum.tiff", units="in", width=4, height=10, res=600)
cols <- c("#fabc3c",
"#a64dd4",
"#14e072",
"#d9014a",
...
points(data.frame(x=rep(2,dim(hum.hvg.warped.kmeans.genes)[1]),y=1000-c(1:dim(hum.hvg.warped.kmeans.genes)[1])*1000/dim(hum.hvg.warped.kmeans.genes)[1]),col=cols[hum.hvg.warped.kmeans.genes$stages], pch=15, cex=0.8)
points(data.frame(x=rep(1,dim(mou.hvg.warped.kmeans.genes)[1]),y=1000-c(1:dim(mou.hvg.warped.kmeans.genes)[1])*1000/dim(mou.hvg.warped.kmeans.genes)[1]),col=cols[mou.hvg.warped.kmeans.genes$stages], pch=15, cex=0.8)
#segments(1,0,10,0,col = "red")
#dev.off()
```
...
ha = rowAnnotation(stages = hum.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(hum.cen.std[as.character(hum.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, left_annotation = ha)
```
...
"#abb4ff",
"#b28e00")
ha = rowAnnotation(stages = mou.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(mou.cen.std[as.character(mou.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, right_annotation = ha)
```
...
ha = rowAnnotation(stages = mou.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(mou.cen.std[as.character(mou.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, left_annotation = ha)
```
...
ha = rowAnnotation(stages = mac.kmeans.genes.ord$stages, col = list(stages = c("1" = cols[1], "2" = cols[2], "3" = cols[3],"4" = cols[4], "5" = cols[5], "6" = cols[6])))
#ha = HeatmapAnnotation(foo = anno_simple(cbind(1:10, 10:1), pch = 1:10))
Heatmap(mac.cen.std[as.character(mac.kmeans.genes.ord$genes),], cluster_rows = FALSE, cluster_columns = FALSE,show_row_names = FALSE, right_annotation = ha)
```
...
Write the genes for 21 cluster kmeans
```{r}
hum.kmeans <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_kmeans/hum.germ.kmean.ortho.list.txt",header = TRUE, fill = TRUE, sep = "\t")
mac.kmeans <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_kmeans/mac.germ.kmean.ortho.list.txt",header = TRUE, fill = TRUE, sep="\t")
mou.kmeans <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_kmeans/mou.germ.kmean.ortho.list.txt",header = TRUE, fill = TRUE,sep="\t")
```
...
for(i in c(1:6)){
  orth.connect[intersect(humMacMouOrth.1to1.in$external_gene_name.x,hum.kmeans[,i]),"humclu"] = i
}

rownames(orth.connect) = humMacMouOrth.1to1.in$mmulatta_homolog_associated_gene_name
for(i in c(1:6)){
  orth.connect[intersect(humMacMouOrth.1to1.in$mmulatta_homolog_associated_gene_name,mac.kmeans[,i]),"macclu"] = i
}

rownames(orth.connect) = humMacMouOrth.1to1.in$mmusculus_homolog_associated_gene_name
for(i in c(1:6)){
  orth.connect[intersect(humMacMouOrth.1to1.in$mmusculus_homolog_associated_gene_name,mou.kmeans[,i]),"mouclu"] = i
}
```

```{r}
write.table(orth.connect, file = "~/storage/HumanMacaqueMouseCompAnaysis/interdata/Germ_kmeans/GermOrthologConnection.txt",quote=FALSE,row.names=FALSE,col.names = TRUE,sep="\t")
```
...
7. Trajectory alignment using Principle curve on the PC1-PC3 space for 200*3 pseduotime points. (1180-1202)
8. Generate 20 clusters for aligned trajectories (1204-1251)
9. Trajectory flow width (number of cells change along the trajectory) (1281-1322)
10. PCA and heatmap for post-aligned 20-cluster-trajectories (1322-1360)
11. K-means gene clustering for three trajectories (1360-1506)
12. Orthologues connecting plots for k-means gene clusters (1508-2029)
