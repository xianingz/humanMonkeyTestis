...
df <- cbind(df, dge.hum@meta.data$res.0.6.ord)
colnames(df) <- c("UMAP1","UMAP2","Cluster")
ggplot(data = df) + geom_point(aes(x=UMAP1, y=UMAP2, col=Cluster)) + scale_color_manual(values = cols[1:9]) +  guides(colour = guide_legend(override.aes = list(size=5))) + theme_bw() +
