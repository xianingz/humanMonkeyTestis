...
```{r}
plotgene_spe("TSPAN33", cells.use = spg.cca@cell.names[spg.cca@meta.data$species=="Human" & spg.cca@meta.data$res.0.6.ord %in% c(1:6)])
```

```{r fig.width=3, fig.height=2.5}
df <- data.frame(expr = spg.cca.hum@data["TSPAN33",], ident = spg.cca.hum@meta.data$res.0.6.ord)
df <- df[df$ident %in% c(1:6),]
ggplot(df, aes(x=ident,y=expr, fill=ident))+geom_violin(scale = "width", adjust=2) + xlab("Identity") + ylab("") + ggtitle("TSPAN33") + theme(legend.position = "none")
```
...
"#0190c8",
"#ffa19e")
df <- data.frame(expr = spg.cca.mac@data["TSPAN33",], ident = spg.cca.mac@meta.data$res.0.6.ord)
df <- df[df$ident %in% c(1:6),]
ggplot(df, aes(x=ident,y=expr, fill=ident))+geom_violin(scale = "width", adjust=1) + xlab("Identity") + ylab("") + ggtitle("TSPAN33")  + scale_fill_manual(values = cols)+ theme(legend.position = "none")
```

```{r}
df <- data.frame(expr = spg.cca.mou@data["TSPAN33",], ident = spg.cca.mou@meta.data$res.0.6.ord)
df <- df[df$ident %in% c(1:6),]
ggplot(df, aes(x=ident,y=expr, fill=ident))+geom_violin(scale = "width", adjust=1/2) + xlab("Identity") + ylab("") + ggtitle("TSPAN33") + theme(legend.position = "none")
```
