...
plts <- list()
glist <- c("SYCP1","SYCP3","MLH1","MLH3","HORMAD1","ZCWPW1","ACRV1","PRM2","PGK2","HSPA1L")
for(i in intersect(glist, humMacMouOrth.1to1.in$external_gene_name.x)){
  df <- data.frame(hum=t(hum.germ.cen[i,]),mac=t(mac.germ.cen[i,]),mou=t(mou.germ.cen[humMacMouOrth.1to1.in$mmusculus_homolog_associated_gene_name[humMacMouOrth.1to1.in$external_gene_name.x==i],]))
  colnames(df) <- c("Human","Monkey","Mouse")
  df <- melt(df)
  df$stages <- rep(c(1:20),3)
