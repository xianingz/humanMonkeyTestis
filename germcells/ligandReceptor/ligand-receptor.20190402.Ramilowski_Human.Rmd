...
#Ligand-receptor analysis for SPG cells and Somatic cells in Human (using ligand/receptor interaction data from Ramilowski_2015)
...
```{r}
soma.mean <- as.matrix(read.table("~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/AllGenes_Markers_Merged4Human-NoC5-NoC1_11celltypes_Qianyi_3.21.2019.txt",header = T,sep="\t",row.names = 1))
soma.mean <- soma.mean[,c(1:7)]
germ.mean <- as.matrix(read.table("~/storage/HumanMacaqueMouseCompAnaysis/interdata/SPG_centroids/HumanSPGCluster_AllGene_Centroids.txt",header = T,sep="\t",row.names = 1))
...
```{r}
lrinter <- read.table("~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/Ramilowski_LigRec_Human.txt",header = TRUE)
ligands <- unique(lrinter$Ligand.ApprovedSymbol)
receptors <- unique(lrinter$Receptor.ApprovedSymbol)
```
...
```{r eval=FALSE}
int.plot(lig.rec.germ.soma, lig.mean = germ.mean, rec.mean = soma.mean, lig.lab = germ.lab, rec.lab = soma.lab, lig.label = "SPG", rec.label = "Soma",lineup = c(14,4), perc=0.2,filename="~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/20190402_SPG_Soma/human/lig.rec.germ.soma.20per.pdf")
int.plot(lig.rec.soma.germ, lig.mean = soma.mean, rec.mean = germ.mean, lig.lab = soma.lab, rec.lab = germ.lab, lig.label = "Soma", rec.label = "SPG",lineup = c(15,4), perc=0.2,filename="~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/20190402_SPG_Soma/human/lig.rec.soma.germ.20per.pdf")
int.plot(lig.rec.soma.soma, lig.mean = soma.mean, rec.mean = soma.mean, lig.lab = soma.lab, rec.lab = soma.lab, lig.label = "Soma", rec.label = "Soma",lineup = c(57,4), perc=0.2,filename="~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/20190402_SPG_Soma/human/lig.rec.soma.soma.20per.pdf")
int.plot(lig.rec.germ.germ, lig.mean = germ.mean, rec.mean = germ.mean, lig.lab = germ.lab, rec.lab = germ.lab, lig.label = "SPG", rec.label = "SPG",lineup = c(3,4), perc=0.2,filename="~/storage/HumanMacaqueMouseCompAnaysis/ligandReceptor/20190402_SPG_Soma/human/lig.rec.germ.germ.20per.pdf")
```